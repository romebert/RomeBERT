...
class BertEncoder(nn.Module):
    def __init__(self, config, ge=False):
        ...
        self.nBlocks = config.num_hidden_layers
        self.ge = ge
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, encoder_hidden_states=None, encoder_attention_mask=None):
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if self.ge:
                highway_logits = gradient_rescale(highway_logits, 1.0 / (self.nBlocks-i))
            
            highway_hidden_states = highway_exit[1]
            highway_entropy = entropy(highway_logits)
            highway_exit = (highway_logits, highway_hidden_states) + (highway_entropy,)  # logits, hidden_states(?), entropy
...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, config, ge=False):
        ...
        self.encoder = BertEncoder(config, ge=ge)
...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def __init__(self, config, ge=False):
        ...
        self.bert = BertModel(config, ge=ge)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
