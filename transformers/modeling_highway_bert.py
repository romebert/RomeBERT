...
class TAKDLoss(nn.Module):
    def __init__(self, nBlocks, gamma, T, num_labels):
        super(TAKDLoss, self).__init__()
    ...
    def forward(self, outputs, highway_outputs, targets):
        if self.num_labels == 1:
            last_loss = self.mse_loss(outputs.view(-1), targets.view(-1))
        else:
            last_loss = self.ce_loss(outputs.view(-1, self.num_labels), targets.view(-1))
        ...
        for i in range(self.nBlocks - 1):
            if self.num_labels == 1:
                _mse = (1. - self.gamma) * self.mse_loss(highway_outputs[i].view(-1), targets.view(-1))
                if i == self.nBlocks - 2:
                    soft_targets = outputs.detach()
                else:
                    soft_targets = highway_outputs[i+1].detach()
                _kld = self.kld_loss(self.log_softmax(highway_outputs[i].view(-1) / T), 
                                     self.softmax(soft_targets.view(-1) / T)) * self.gamma * T * T
                multi_losses.append(_mse)
                distill_losses.append(_kld)
...
class GroupKDLoss(nn.Module):
    def __init__(self, nBlocks, gamma, beta, irange, T, num_labels):
        super(GroupKDLoss, self).__init__()
        self.kld_loss = nn.KLDivLoss().cuda()
        self.ce_loss = nn.CrossEntropyLoss().cuda()
        self.mse_loss = nn.MSELoss().cuda()
        self.log_softmax = nn.LogSoftmax(dim=1).cuda()
    ...
    def forward(self, outputs, highway_outputs, targets, soft_targets):
        ...
        multi_losses = []
        distill_losses = []
        for i in range(self.nBlocks - self.irange):
            if self.num_labels == 1:
                _sum_outputs = sum(highway_outputs[i:i+self.irange]) / self.irange
                _mse = self.beta * self.mse_loss(_sum_outputs.view(-1), targets.view(-1))
                multi_losses.append(_mse)
            else:
                _sum_outputs = sum(highway_outputs[i:i+self.irange])
                _ce = self.beta * self.ce_loss(_sum_outputs.view(-1, self.num_labels), targets.view(-1))
                multi_losses.append(_ce)
                
        for j in range(self.nBlocks - 1):
            if self.num_labels == 1:
                _kld = self.kld_loss(self.log_softmax(highway_outputs[j].view(-1) / T), 
                                     self.softmax(soft_targets.view(-1) / T)) * self.gamma * T * T
                distill_losses.append(_kld)
            else:
                _kld = self.kld_loss(self.log_softmax(highway_outputs[j].view(-1, self.num_labels) / T), 
                                     self.softmax(soft_targets.view(-1, self.num_labels) / T)) * self.gamma * T * T
                distill_losses.append(_kld)
...
class BidirectionLoss(nn.Module):
    def __init__(self, nBlocks, gamma, num_labels):
        super(BidirectionLoss, self).__init__()
    ...
    def forward(self, outputs, highway_outputs, targets, soft_targets):
        ...
        for i in range(self.nBlocks - 1):
            if self.num_labels == 1:
                _mse = (1. - self.gamma) * self.mse_loss(highway_outputs[i].view(-1), targets.view(-1))
                _t_loss = -(highway_outputs[i].view(-1) * soft_targets.view(-1)).sum() * self.gamma
                multi_losses.append(_mse)
                distill_losses.append(_t_loss)
            else:
                _ce = (1. - self.gamma) * self.ce_loss(highway_outputs[i].view(-1, self.num_labels), targets.view(-1))
                _t_loss = -(highway_outputs[i].view(-1, self.num_labels) * soft_targets.view(-1, self.num_labels)).sum() * self.gamma
                multi_losses.append(_ce)
                distill_losses.append(_t_loss)
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, encoder_hidden_states=None, encoder_attention_mask=None):
        ...
        return outputs  # last-layer hidden state, (all hidden states), (all attentions), all highway exits (optional)
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None,
        ...
        return outputs  # sequence_output, pooled_output, (hidden_states), (attentions), highway exits (optional)
...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def use_slow_teacher(self):
        if not self.moco:
            self.moco = True
            self.slow_bert = BertModel(self.config, slow=True)
    ...
    def set_momentum_value(self, value=0.9999):
        if self.m is None:
            self.m = value
            print('Setting the value of MoCo momentum to {}.'.format(self.m))
            
    def _slow_teacher_initialization(self):
        for param_q, param_k in zip(self.bert.parameters(), self.slow_bert.parameters()):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None,
                ...
                output_layer=-1, kd_loss_type='raw', gamma=0.9, beta=None, irange=None, temper=3.0):
        ...
        if self.moco:
            ...
            with torch.no_grad():  # no gradient to keys
                ...
                slow_outputs = self.slow_bert(input_ids,
                                attention_mask=attention_mask,
                                token_type_ids=token_type_ids,
                                position_ids=position_ids,
                                head_mask=head_mask,
