...
def train(args, train_dataset, model, tokenizer, kd_loss_type='raw'):
    ...
    show_tag = True # Don't show all params
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if not show_tag:
                ...
                for name, param in model.named_parameters():
                    ...
                    #if 'encoder.layer' in name and 'layer.11' not in name and param.grad is not None:
                    '''for moco slow teacher'''
                    if ('embeddings' in name or 'bert.pooler' in name or name in ['classifier.weight', 'classifier.bias'] or 
                        'encoder.layer' in name) and param.grad is not None:
                        print(name)
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                if type(tmp_eval_loss) == tuple:
                    tmp_eval_loss = tmp_eval_loss[0] # full_loss
...
def main():
    ...
    ### add moco hyperparameters
    parser.add_argument("--moco", action='store_true', help="whether to use MoCo slow teacher")
    parser.add_argument('--momentum', type=float, default=0.9999, help='the momentum factor')
    ...
    if args.model_type == "bert":
        ...
        if args.moco:
            model.set_momentum_value(args.momentum)
            model.use_slow_teacher()
    else:
