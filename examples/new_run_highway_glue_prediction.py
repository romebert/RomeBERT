...
from transformers.new_modeling_highway_bert import BertForSequenceClassification
from transformers.new_modeling_highway_roberta import RobertaForSequenceClassification
...
def train(args, train_dataset, model, tokenizer, kd_loss_type='raw'):
    """ Train the model """
    ...
    tr_last_loss, logging_last_loss = 0.0, 0.0
    tr_multi_loss, logging_multi_loss = 0.0, 0.0
    model.zero_grad()
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs['kd_loss_type'] = kd_loss_type
            inputs['gamma'] = args.gamma
            inputs['beta'] = args.beta
            inputs['irange'] = args.irange
            inputs['temper'] = args.temper
            outputs = model(**inputs)
            loss = outputs[0]  # model outputs are always tuple in transformers (see doc)
            if type(loss) == tuple:
                if args.task_name in ["mnli", "mnli-mm", "qqp"]:
                    ### origin ###
                    last_loss = loss[1]
                    multi_loss = loss[2] + loss[3]
                    full_loss = loss[0]
                else:
                    ### reverse ###
                    last_loss = loss[2] + loss[3]
                    multi_loss = loss[1]
                    full_loss = loss[0]
            
            if args.n_gpu > 1:
                last_loss = last_loss.mean() # mean() to average on multi-gpu parallel training
                multi_loss = multi_loss.mean()
                full_loss = full_loss.mean()
                
            if args.gradient_accumulation_steps > 1:
                last_loss = last_loss / args.gradient_accumulation_steps
                multi_loss = multi_loss / args.gradient_accumulation_steps
                full_loss = full_loss / args.gradient_accumulation_steps

            if args.gp:
                if args.fp16:
                    with amp.scale_loss(last_loss, optimizer) as scaled_loss:
                ...
                if args.fp16:
                    with amp.scale_loss(multi_loss, optimizer) as scaled_loss:
                ...
                cos_v =  F.cosine_similarity(grad_list_last.unsqueeze(0), grad_list_multi.unsqueeze(0))
                angle = torch.acos(cos_v) / 3.1415 * 180.0
                #print('angle: ', torch.acos(cos_v) / 3.1415 * 180.0)
                l2norm_last = torch.norm(grad_list_last)
                l2norm_multi = torch.norm(grad_list_multi)
                norm_ratio = l2norm_last / l2norm_multi
                #print l2norm for grad_list_last & grad_list_multi
                ...
                grad_list_multi = grad_list_multi - torch.min(proj_direction, torch.zeros([1]).cuda()) * grad_list_last
            ...
            tr_last_loss += last_loss.item()
            tr_multi_loss += multi_loss.item()
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.gp:
                    tb_writer.add_scalar('angle', angle.item(), global_step)
                    tb_writer.add_scalar('grad_magnitude_last', l2norm_last.item(), global_step)
                    tb_writer.add_scalar('grad_magnitude_multi', l2norm_multi.item(), global_step) 
                    tb_writer.add_scalar('norm_ratio', norm_ratio.item(), global_step)
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    tb_writer.add_scalar('last_loss', (tr_last_loss - logging_last_loss)/args.logging_steps, global_step)
                    tb_writer.add_scalar('multi_loss', (tr_multi_loss - logging_multi_loss)/args.logging_steps, global_step)
                    logging_loss = tr_loss
                    logging_last_loss = tr_last_loss
                    logging_multi_loss = tr_multi_loss
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                inputs['kd_loss_type'] = args.kd_loss_type
                inputs['gamma'] = args.gamma
                inputs['beta'] = args.beta
                inputs['irange'] = args.irange
                inputs['temper'] = args.temper
        ...
        test_pred.to_csv(eval_task + '.tsv', sep='\t', index=False)
...
def main():
    ...
    parser.add_argument('--beta', type=float, default=None, help='beta for group-kld loss')
    parser.add_argument('--irange', type=int, default=None, help='inference range for group-kld loss')
    parser.add_argument('--temper', type=float, default=3.0, help='Temperature for KD')
    parser.add_argument('--kd_loss_type', type=str, choices=['raw', 'group', 'ta', 'bidir', 'other'], default='raw', help='distillation loss type')
    
    ### add gradient hyperparameters
    parser.add_argument("--ge", action='store_true', help="whether to use gradient equilibrium")
    parser.add_argument("--gp", action='store_true', help="whether to use gradient projection")
    ...
    print('Task name is {}.'.format(args.task_name))
    print('Task in MNLI & QQP: {}.'.format(args.task_name in ["mnli", "mnli-mm", "qqp"]))
    ...
    if args.do_train:
        ...
        global_step, tr_loss = train(args, train_dataset, model, tokenizer, kd_loss_type=args.kd_loss_type)
